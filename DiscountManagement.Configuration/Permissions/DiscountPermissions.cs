namespace DiscountManagement.Configuration.Permissions;
public static class DiscountPermissions
{
    // ColleagueDiscount Permissions
    public const int DefineColleagueDiscount = 150;
    public const int EditColleagueDiscount = 151;
    public const int SearchColleagueDiscounts = 152;
    public const int RemoveColleagueDiscount = 153;
    public const int RestoreColleagueDiscount = 154;

    // CustomerDiscount Permissions
    public const int DefineCustomerDiscount = 160;
    public const int EditCustomerDiscount = 161;
    public const int SearchCustomerDiscounts = 162;
}
